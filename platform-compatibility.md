...
Deploying [`.NET`](api/table.md#dotnet) code to mobile platforms meant dealing with various compatibility issues. This document lists the issues found while deploying of [`.NET`](api/table.md#dotnet) code to iOS, Windows Phone and Android. The deployment was done using a tool called Unity, which is a Mono-based game engine.

The project during which this was done was [`JJ.SaveText`](https://github.com/jjvanzon/JJ.SaveText).

Some work-arounds had been given a place in [`JJ.Framework.PlaformCompatibility`](https://www.nuget.org/packages/JJ.Framework.PlatformCompatibility) to keep overview of these issues. But that package may not be relevant for later [`.NET`](api/table.md#dotnet) versions anymore.


Unity 4.3.4 / .NET Compatibility Issues
---------------------------------------

The following things might need to be taken into consideration to make the [`.NET`](api/table.md#dotnet) code work in Unity.

- ### Use .NET 3.5

  Unity / Mono (as of 2014-05-17) appears to be only compatible with .NET 3.5, not 4.0 or 4.5, so things were targeted to .NET 3.5 in the [`.NET`](api/table.md#dotnet) code and workarounds were made for missing features.

- ### Copy your own Compiled Assemblies

  To use self-made [`.NET`](api/table.md#dotnet) assemblies in Unity, they were copied to the `Assets` folder or a sub-folder in there in the Unity project.
...
- ### System.Net.Sockets
  ...
  Unity Free did not support deploying applications that use `System.Net.Sockets`. That only seemed available in Unity Pro. This also means any part of [`.NET`](api/table.md#dotnet) that indirectly uses `System.Net.Sockets`. So basically connecting to a network from the [`.NET`](api/table.md#dotnet) code is disabled. The emulator in Unity could use it, but deploying it to a device did not work. To make the deployment succeed, removing `System.ServiceModel.dll` from the `Assets` folder might work and anything else that uses `System.Net.Sockets`.
...
- ### System.Diagnostics.Trace Class
  ...
  Not available on Windows Phone 8. (This made [`JJ.Framework.Logging`](api/table.md#jj-framework-logging) not usable.) 
...
- ### JIT Compilation
  ...
  Any JIT compilation might not be permitted on iOS. Some functions of [`.NET`](api/table.md#dotnet) (or the Mono versions of `.NET`) automatically seem to JIT something on the fly.
...
- ### Compile for Release
  ...
  Resources might not work when you do not compile [`.NET`](api/table.md#dotnet) assemblies for release.
