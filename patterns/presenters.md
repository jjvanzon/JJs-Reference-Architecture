...
<img src="../images/presenter-code.png" width="800" />
...
Unlike the [`ViewModels`](viewmodels.md), which model the *data* shown on a screen, the [`Presenters`](#-presenters) model the *actions* a user can perform on the screen.
...
Our action methods of the [`Presenter`](#-presenters) work by a [`ViewModel-in`](viewmodels.md), [`ViewModel-out`](viewmodels.md) principle. An action method returns a [`ViewModel`](viewmodels.md) that contains the data to display on screen:
...
Action methods can also *receive* a [`ViewModel`](viewmodels.md) parameter containing the data the user has edited:
...
The [`Presenter's`](#-presenters) action methods are not limited to returning a single type of [`ViewModel`](viewmodels.md). It is possible to return different [`ViewModel`](viewmodels.md) types as well:
...
It is preferred that [`ViewModel`](viewmodels.md) creation is delegated to the [`ToViewModel`](presentation.md#toviewmodel) layer (instead of creating them directly within the [`Presenters`](#-presenters)):
...
This way, [`ViewModel` creation](viewmodels.md) is in a centralized spot, so that changes only have to be made in one place.


Delegating More Responsibilities
--------------------------------

The responsibility of the [`Presenter`](#-presenters) is not limited to [`ViewModel`](viewmodels.md) creation alone, as you can see here:
...
This way the [`Presenter`](#-presenters) layer is also responsible for [retrieving data](../layers.md#data-layer), calling [business logic](../layers.md#business-layer) and converting [`ViewModels`](viewmodels.md) back to [`Entities`](data-access.md#entities). So it isn't a needless [pass-through layer](../practices-and-principles.md#pass-through-layers-) just delegating [`ViewModel`](viewmodels.md) creation. It is a [combinator `class`](business-logic.md#facade), in that it combines multiple smaller aspects of the logic, by delegating work to various parts of the system.
...
| [`ViewModel`](viewmodels.md) [`Validation`](business-logic.md#validators) | If possible, [`Entity`](data-access.md#entities) [`Validation`](business-logic.md#validators) is preferred. But sometimes the data can't be converted to an [`Entity`](data-access.md#entities) yet. [`Validating`](business-logic.md#validators) a [`ViewModel`](viewmodels.md) might be useful in that case. 
| [`ToEntity`](presentation.md#toentity) | Converting [ViewModel](viewmodels.md) data into [`Entity`](data-access.md#entities) data.
...
| [`ToViewModel`](presentation.md#toviewmodel) | Mapping [`Entity`](data-access.md#entities) data to the corresponding [`ViewModel`](viewmodels.md) `properties`, to prepare it for the [`view`](presentation.md#views).
| `NonPersisted Data` | Copying data that does not need to be stored, from the old [`ViewModel`](viewmodels.md) to the new, such as selections or search criteria.
| `Redirect` | Returning a different [`ViewModel`](viewmodels.md), to trigger the UI to go to the appropriate page like a success message or the home screen.
...
Even though the actual call to the [business logic](../layers.md#business-layer) might be trivial, it may still be necessary to convert from [`Entity`](data-access.md#entities) to [`ViewModel`](viewmodels.md) and back.
...
You might save the server some work by doing *partial loads* instead of *full loads* or maybe execute client-native code. For more info, see: [Full Load – Partial Load – Cient-Native Code](presentation.md#full-load--partial-load--client-native-code).


Using ViewModels Directly
-------------------------

Some actions might also operate onto [`ViewModels`](viewmodels.md) directly instead:
...
The [`Presenter`](#-presenters) pattern is a commonly used design pattern for modeling user interactions in an application. By creating a [`Presenter`](#-presenters) for each [`View`](presentation.md#views) and working with [`ViewModels`](viewmodels.md), we can achieve a clear modularization of our [presentation logic](../layers.md#presentation-layer) and we ensure that each component has a specific responsibility. Delegating [`ViewModel`](viewmodels.md) creation to the [`ToViewModel`](presentation.md#toviewmodel) layer enables separation of concerns and allows the [`Presenter`](#-presenters) to focus on its primary responsibility of modeling user interaction and delegating work to the various parts of the system.

The [`Presenters`](#-presenters) form a [platform-independent](../layers.md#platform-independence-1) layer below the actual front-end technology. All logic is hidden under a shell of [`ViewModels`](viewmodels.md) and user actions. This makes it possible to swap out the [front-end](../layers.md#platform-independence-1) while leaving the underlying system intact.
