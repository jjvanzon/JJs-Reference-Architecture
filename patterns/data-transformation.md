...
When converting one type to another one might use the `TryGet-Insert-Update` pattern. Especially when converting an [`Entity`](data-access.md#entities) with related [`Entities`](data-access.md##entities) from one structure to another this pattern will make the code easier to read.

[`TryGet`](other.md#tryget) first gets a possible existing destination [`Entity`](data-access.md#entities).

`Insert` will create the [`Entity`](data-access.md#entities) if it did not exist yet, possibly setting some defaults.

`Update` will update the rest of the properties of either the existing or newly created `object`.

When you do these actions one by one for one destination [`Entity`](data-access.md#entities) after another, you will get readable code for complex conversions between data structures.
...
- [`TryGet`](other.md#tryget): look up an item in the destination collection.
...
[`MVC`](../api/table.md#mvc) itself contains a specialized version of this very pattern. The following layering stacks are completely analogous to eachother:
