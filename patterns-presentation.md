...
The `ScreenViewModel class` defines common properties that can be used for any screen or page, such as the title and visibility. The `ValidationViewModel class` has properties for data validation on a page. The `HomePage` uses common properties and has a `Login`. The `ProductEdit` view uses common properties and has `Validation`.
