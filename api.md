...
When using these `API's`, transactions might not work unless you enable `MSDTC` (**M**icrosoft **D**istributed **T**ransaction **C**oordinator). That would be a `Windows` service belonging to an [`SQL Server`](https://www.microsoft.com/en-us/sql-server) installation.
